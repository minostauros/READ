...
class DukeVideoReID(Dataset):
  ...
  URL: `<https://github.com/Yu-Wu/DukeMTMC-VideoReID/blob/f8d34254e3ef1a563e405801327cd3e4691ad9f1/README.md#directory-structure>`_
  
  Dataset statistics:
      - identities: 702 for training, 702 for testing, and 408 as distractors
      - tracklets: 2,196 videos for training and 2,636 videos for testing
      - cameras: 6.
  """
  def __init__(self, 
    json_file,
    ...
    # if for training, divide TRAINING set separated by person_ids
    ...
    self.h5_path = os.path.join(
      self.h5_dir, 'dukemtmc_tracklets_{}.h5'.format(self.subset)
      )
    ...
    print('DukeVideoReID is initialized. '
  ...
  def __getitem__(self, idx):
    ...
    if self.subset == 'train':
      ...
      sample = {
        'images': torch.stack([s['images'] for s in samples], dim=0), # (samples_per_batch, sample_length, C, H, W)
...
def DukeVideoReIDLoader(
  ...
  if len(kwargs) > 0:
    print('DukeVideoReIDLoader: arguments {} are ignored.'.format(kwargs))

  # load dataset
  dataset = DukeVideoReID(
    json_file=json_file, 
    h5_dir=h5_dir, 
    subset=subset,
